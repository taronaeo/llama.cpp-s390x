...
static void ggml_zdnn_mul_mat_op(ggml_backend_zdnn_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    ggml_zdnn_create_tensor(ptd_weights, td_weights, zt_weights, weights, weights_dim, ZDNN_2D);
    ggml_zdnn_create_tensor(ptd_inputs,  td_inputs,  zt_inputs,  inputs,  inputs_dim,  ZDNN_2D);
    ...
    ggml_zdnn_load_tensor(zt_weights, weights->data);
    ...
    ZDNN_CHECK(zdnn_transform_origtensor(&zt_output, output->data));

    ZDNN_CHECK(zdnn_free_ztensor_buffer(&zt_weights));
    ZDNN_CHECK(zdnn_free_ztensor_buffer(&zt_inputs));
    ZDNN_CHECK(zdnn_free_ztensor_buffer(&zt_bias));
    ZDNN_CHECK(zdnn_free_ztensor_buffer(&zt_output));
...
static void ggml_backend_zdnn_buffer_free_buffer(ggml_backend_buffer_t buffer) {
    ...
    for (int i = 0; i < ctx->n_buffers; i++) {
        GGML_LOG_DEBUG("%s: freeing ztensor buffer %d at %p\n", __func__, i, ctx->buffers[i].ztensor.buffer);
